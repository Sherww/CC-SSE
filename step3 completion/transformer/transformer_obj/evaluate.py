...
def translate(model, sentence, source_tokenizer, target_tokenizer, target_max_seq_len=8, 
    ...
    # !!!!!!!!!!!!
    source_tokens = source_tokenizer.encode(sentence, truncation=True, max_length=configs["source_max_seq_len"])
...
def calculate_bleu_score(model, source_tokenizer, target_tokenizer, configs):
    ...
    # with open("logs/predict_test.txt", "w",encoding='utf-8') as f:
    #     for i in range(len(pred_sents)):
    #         f.write(pred_sents[i]+'\n')

    # hypotheses = [preprocess_seq(sent).split() for sent in pred_sents]
    # references = [[sent.split()] for sent in valid_trg_data]
    
    # weights = [(1),(0.5, 0.5),(0.333, 0.333, 0.334),(0.25, 0.25, 0.25, 0.25)]
    # bleu_1 = corpus_bleu(references, hypotheses, weights=weights[0])

    # # bleu_2 = corpus_bleu(references, hypotheses, weights=weights[1])
    # # bleu_3 = corpus_bleu(references, hypotheses, weights=weights[2])
    # # bleu_4 = corpus_bleu(references, hypotheses, weights=weights[3])
...
def main():
    ...
    pred_sents=calculate_bleu_score(model, source_tokenizer, target_tokenizer, configs)
...
if __name__ == "__main__":
    pred_sents=main()
    # write prediction to file
    with open("logs/predict_test_obj.txt", "w",encoding='utf-8') as f:
        for i in range(len(pred_sents)):
            f.write(pred_sents[i]+'\n')