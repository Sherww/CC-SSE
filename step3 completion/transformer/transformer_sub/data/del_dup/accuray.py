...
def hebing_sub():
    with open('result_sub_deldup.txt','r',encoding='utf-8')as f:
        data=f.readlines()
        rels=[]
        for dat in data:
            dat=dat.replace('.',' ')
            ...
            # rel=dat.split('\t')[1]
            rels.append(nee)
    with open('test_sub_input_deldup.txt','r',encoding='utf-8')as ff:
        data1=ff.readlines()
    with open('test_sub_output_deldup.txt','w',encoding='utf-8')as fff:
        
        for i in range(len(rels)):
            fff.write(data1[i].replace('\n','')+ ' '+rels[i]+'\n')

def sub():
    with open('test_sub_output_deldup.txt','r',encoding='utf-8')as f:
        data=f.readlines()
        result=[]
        nees=[]
        codes=[]
    ...
    print(len(nees))
    print(len(codes))
    with open('test_sub_deldup.txt','r',encoding='utf-8')as f1:
        data=f1.readlines()
        test=[]
        nee2s=[]
        codes2=[]
